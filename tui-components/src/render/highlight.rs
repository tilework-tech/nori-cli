//! Syntax highlighting support for bash scripts using tree-sitter.
//!
//! This module is only available when the `syntax-highlighting` feature is enabled.
//! It provides bash syntax highlighting capabilities using tree-sitter's highlight
//! queries, converting bash scripts into styled Ratatui lines.
//!
//! # Examples
//!
//! ```rust
...
//! "#;
//!
//! let styled_lines = highlight_bash_to_lines(script);
//! // Returns Vec<Line<'static>> with syntax highlighting applied
//! # }
//! ```
...
/// Bash syntax categories mapped from tree-sitter highlight queries.
///
...
/// Converts a bash script into styled lines using tree-sitter syntax highlighting.
///
/// This function parses bash scripts and applies syntax highlighting based on
/// tree-sitter's bash highlight queries. The highlighter is streamed so multi-line
/// content is split into [`Line`]s while preserving style boundaries.
///
/// Dimmed styles are applied to comments, operators, and strings to improve readability.
///
/// # Arguments
///
/// * `script` - The bash script source code to highlight
///
/// # Returns
///
/// A vector of styled lines. If highlighting fails, returns the script as plain text.
///
/// # Examples
///
/// ```rust
...
///
/// // Operators like "&&" and "|" will be dimmed
/// // Commands like "echo" and "ls" will have normal style
/// # }
/// ```
