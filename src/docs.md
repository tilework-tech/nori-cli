...
- `BACKEND_OPTIONS`: Centralized constant containing all backend metadata (name, factory function) - eliminates multiple disconnected sources for backend ordering that could get out of sync
- `BackendOption` struct: Contains `name: &'static str` and `factory: fn() -> Box<dyn AgentBackend + Send>` - availability checking delegates to the backend's `is_available()` trait method
