...
**Single-file header deduplication**: When an edit has exactly 1 file with diff data (and is not a Move), the `DiffSummary`'s first header line (which includes verb+path+line counts, e.g., "Edited README.md (+1 -1)") is promoted to the outer header position. This eliminates the visual duplication where the outer header said "Edited README.md" and the nested `DiffSummary` repeated "Edited README.md (+1 -1)". The promoted line inherits the phase-aware bullet styling (green bold for completed, red bold for failed). Multi-file edits, failed edits without diff data, and Move tools retain the original two-level header behavior — Move tools are excluded because the `DiffSummary` says "Edited" while the tool header says "Moved".
