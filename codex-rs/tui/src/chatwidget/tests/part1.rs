...
fn empty_enter_during_task_does_not_queue() {
    ...
    // Ensure no op was sent to the backend.
...
fn streaming_final_answer_keeps_task_running_state() {
    ...
    // Since queue_user_message now submits directly to the backend, verify the
    // Op::UserInput was sent.
    assert_matches!(op_rx.try_recv(), Ok(Op::UserInput { .. }));
    // Also consume the AddToHistory op that follows.
    assert_matches!(op_rx.try_recv(), Ok(Op::AddToHistory { .. }));
