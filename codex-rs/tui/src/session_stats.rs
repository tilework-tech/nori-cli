...
mod tests {
    ...
    // =========================================================================
    // Tests for to_display_string (console output)
    // =========================================================================

    #[test]
    fn to_display_string_shows_title() {
        let stats = SessionStats::new();
        let output = stats.to_display_string();
        assert!(
            output.contains("Nori Session Statistics"),
            "Expected title in output"
        );
    }

    #[test]
    fn to_display_string_shows_message_counts() {
        let mut stats = SessionStats::new();
        stats.user_messages = 3;
        stats.assistant_messages = 5;
        let output = stats.to_display_string();
        assert!(output.contains("User: 3"), "Expected user count");
        assert!(output.contains("Assistant: 5"), "Expected assistant count");
    }

    #[test]
    fn to_display_string_shows_tool_calls() {
        let mut stats = SessionStats::new();
        stats.tool_calls.insert("Bash".to_string(), 2);
        let output = stats.to_display_string();
        assert!(output.contains("Bash: 2"), "Expected Bash count");
    }

    #[test]
    fn to_display_string_shows_skills() {
        let mut stats = SessionStats::new();
        stats.skills_used.push("tdd".to_string());
        let output = stats.to_display_string();
        assert!(output.contains("tdd"), "Expected tdd skill");
    }

    #[test]
    fn to_display_string_shows_subagents() {
        let mut stats = SessionStats::new();
        stats
            .subagents_used
            .push("nori-codebase-locator".to_string());
        let output = stats.to_display_string();
        assert!(
            output.contains("nori-codebase-locator"),
            "Expected subagent"
        );
    }
