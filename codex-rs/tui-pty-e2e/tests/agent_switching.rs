...
// ============================================================================
// Test: Agent Switch Failure Recovery (Graceful Error Handling)
// ============================================================================

/// Test that starting with an invalid agent in config.toml does not cause a crash loop.
///
/// This is a regression test for the crash loop bug where:
/// - Config was persisted BEFORE spawn attempt
/// - Spawn failure sent ExitRequest, causing CLI to exit
/// - On restart, failed agent was loaded from config → crash loop
///
/// The fix should:
/// 1. Not persist config until spawn succeeds, OR
/// 2. Gracefully handle invalid agent on startup (fallback to default)
///
/// CURRENT BEHAVIOR (broken): CLI exits after showing error
/// EXPECTED BEHAVIOR (fixed): CLI shows error but remains interactive
#[test]
#[cfg(target_os = "linux")]
fn test_startup_with_invalid_agent_remains_interactive() {
    // Create a config with an agent that will fail to spawn
    // We use "codex" which exists in the registry but will fail because
    // the codex binary is not installed in the test environment
    let config = SessionConfig::new().with_model("codex".to_string()); // This will fail to spawn

    let mut session = TuiSession::spawn_with_config(24, 80, config).expect("Failed to spawn TUI");

    // Wait for either:
    // 1. The prompt to appear (good - CLI is interactive)
    // 2. An error message to appear (expected - spawn fails)
    let initial_result = session.wait_for(
        |screen| {
            screen.contains("›")
        ...
        Duration::from_secs(30), // Give npx time to try and fail
    );

    // Even if the initial wait times out, let's check the state
    let screen_after_wait = session.screen_contents();
    eprintln!(
        "Screen after initial wait (result: {:?}): \n{}",
        initial_result.is_ok(),
        screen_after_wait
    );

    // Now wait longer to see if the CLI exits (which is the bug)
    // If the fix is in place, the CLI should stay running
    std::thread::sleep(Duration::from_secs(3));

    // Try to interact - if CLI exited, this will fail
    let send_result = session.send_str("test");
    if send_result.is_err() {
        // CLI has exited - this is the BROKEN behavior
        // The test should FAIL here until the fix is implemented
        panic!(
            "CLI exited after agent spawn failure - this is the bug! \
             Expected CLI to remain interactive. Last screen:\n{}",
            screen_after_wait
        );
    }

    // Check if the prompt is still responsive
    std::thread::sleep(TIMEOUT_INPUT);
    let _ = session.poll();
    let final_screen = session.screen_contents();

    // The CLI should still be showing something (not exited)
    let still_running = !final_screen.is_empty()
        && (final_screen.contains("›")
            || final_screen.contains("test")
            || final_screen.contains("error")
            || final_screen.contains("Error"));

    assert!(
        still_running,
        "CLI should remain interactive after agent spawn failure. Final screen:\n{}",
        final_screen
    );
}

/// Test that config is persisted AFTER successful spawn (SessionConfigured),
/// not before.
///
/// This is a regression test for the bug where:
/// 1. User selects new agent via /agent picker
/// 2. Config was persisted with new agent BEFORE spawn attempt (BUG)
/// 3. If spawn fails, CLI would crash loop on restart
///
/// The fix ensures config is only persisted AFTER SessionConfigured is received.
///
/// Note: This test uses Codex which DOES successfully spawn (sends SessionConfigured),
/// so the config SHOULD be persisted. This verifies the positive case - that persistence
/// still works correctly after the fix.
#[test]
#[cfg(target_os = "linux")]
fn test_agent_switch_config_persisted_after_successful_spawn() {
    let config = SessionConfig::new().with_model("mock-model".to_string());
    ...
    // Get the NORI_HOME path to check config later
    ...
    // Read initial config
    let initial_config =
        std::fs::read_to_string(&config_path).expect("Should be able to read config.toml");
    eprintln!("Initial config:\n{}", initial_config);

    // Verify initial config has mock-model
    assert!(
        initial_config.contains("mock-model"),
        "Initial config should have mock-model"
    );

    // Verify initial agent is working
    session.send_str("hello").unwrap();
    std::thread::sleep(TIMEOUT_INPUT);
    session.send_key(Key::Enter).unwrap();
    ...
    // Navigate to select Codex
    // The picker order is: Mock ACP, Mock ACP Alt, Claude, Codex, Gemini
    // So we need to go down 3 times to reach Codex
    session.send_key(Key::Down).unwrap();
    std::thread::sleep(TIMEOUT_INPUT);
    session.send_key(Key::Down).unwrap();
    std::thread::sleep(TIMEOUT_INPUT);
    ...
    // Submit a prompt to trigger the actual agent switch attempt
    session.send_str("trigger switch").unwrap();
    std::thread::sleep(TIMEOUT_INPUT);
    session.send_key(Key::Enter).unwrap();

    // Wait for the spawn attempt - Codex agent will spawn successfully (sends SessionConfigured)
    // even though it will fail authentication later
    std::thread::sleep(Duration::from_secs(10));

    // Read config after switch attempt
    let config_after_switch =
        std::fs::read_to_string(&config_path).expect("Should be able to read config.toml");
    eprintln!("Config after switch attempt:\n{}", config_after_switch);

    // Since Codex DOES successfully spawn (SessionConfigured is received),
    // the config SHOULD be persisted with "codex". This verifies that:
    // 1. Config persistence still works after the fix
    // 2. Persistence happens after SessionConfigured, not before spawn
    let config_has_codex = config_after_switch.contains("agent = \"codex\"");

    assert!(
        config_has_codex,
        "Config SHOULD be updated to 'codex' after successful spawn. \
         SessionConfigured was received, so config should be persisted. \
         Config contents:\n{}",
        config_after_switch
    );
}

/// Test that switching to mock-model-alt (which always spawns successfully)
/// persists the config correctly.
///
/// This verifies that the normal agent switch flow still works after the fix.
#[test]
#[cfg(target_os = "linux")]
fn test_agent_switch_to_mock_alt_persists_config() {
    let config = SessionConfig::new().with_model("mock-model".to_string());
    ...
    // Verify initial config has mock-model (not mock-model-alt)
    assert!(
        initial_config.contains("mock-model"),
        "Initial config should have mock-model"
    );
    assert!(
        !initial_config.contains("mock-model-alt"),
        "Initial config should NOT have mock-model-alt"
    );
    ...
    // Navigate to select Mock ACP Alt (second in the list)
    session.send_key(Key::Down).unwrap();
    std::thread::sleep(TIMEOUT_INPUT);
    session.send_key(Key::Enter).unwrap();
    std::thread::sleep(Duration::from_millis(500));

    // Submit a prompt to trigger the actual agent switch
    session.send_str("test switch to alt").unwrap();
    std::thread::sleep(TIMEOUT_INPUT);
    session.send_key(Key::Enter).unwrap();

    // Wait for response from new agent
    session
        .wait_for_text("Test message", Duration::from_secs(5))
        .expect("New agent should respond");

    // Read config after successful switch
    let config_after_switch =
        std::fs::read_to_string(&config_path).expect("Should be able to read config.toml");
    eprintln!("Config after switch:\n{}", config_after_switch);

    // Config should now have mock-model-alt
    assert!(
        config_after_switch.contains("mock-model-alt"),
        "Config should be updated to 'mock-model-alt' after successful switch. \
         Config contents:\n{}",
        config_after_switch
    );
}
