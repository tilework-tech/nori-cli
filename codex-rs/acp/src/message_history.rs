...
//!
//! The history is stored at `~/.nori/cli/history.jsonl` with **one JSON object per
...
/// Filename that stores the message history inside `~/.nori/cli`.
const HISTORY_FILENAME: &str = "history.jsonl";

const MAX_RETRIES: usize = 10;
const RETRY_SLEEP: Duration = Duration::from_millis(100);

/// A single entry in the message history file.
...
pub fn history_filepath(nori_home: &Path) -> PathBuf {
    nori_home.join(HISTORY_FILENAME)
}

/// Append a `text` entry associated with `conversation_id` to the history file.
///
/// Uses advisory file locking to ensure that concurrent writes do not interleave,
/// which entails a small amount of blocking I/O internally.
///
/// # Arguments
/// * `text` - The message text to append
/// * `conversation_id` - The conversation/session ID to associate with this entry
...
pub async fn append_entry(
    ...
    // Resolve history file path and ensure the parent directory exists.
...
pub async fn history_metadata(nori_home: &Path) -> (u64, usize) {
    let path = history_filepath(nori_home);
...
/// the current history file **and** the requested offset exists.
///
/// Any I/O or parsing errors are logged and result in `None`.
///
/// Note this function is not async because it uses a sync advisory file
/// locking API.
#[cfg(any(unix, windows))]
pub fn lookup(log_id: u64, offset: usize, nori_home: &Path) -> Option<HistoryEntry> {
    let path = history_filepath(nori_home);
    lookup_history_entry(&path, log_id, offset)
}

/// On Unix systems, ensure the file permissions are `0o600` (rw-------).
/// If the permissions cannot be changed the error is propagated to the caller.
#[cfg(unix)]
...
mod tests {
    ...
    #[tokio::test]
    async fn test_append_entry_creates_history_file() {
        ...
        // Verify content
        let content = std::fs::read_to_string(&history_path).expect("read history file");
        let entry: HistoryEntry = serde_json::from_str(content.trim()).expect("parse entry");
        assert_eq!(entry.text, "Hello, world!");
        assert_eq!(entry.session_id, conversation_id.to_string());
    }

    #[tokio::test]
    async fn test_append_entry_with_persistence_none_does_nothing() {
        let temp_dir = TempDir::new().expect("create temp dir");
        let conversation_id = ConversationId::new();
        ...
        let history_path = history_filepath(temp_dir.path());
        assert!(!history_path.exists(), "history file should NOT exist");
    }
    ...
    async fn test_append_multiple_entries() {
        ...
        for (i, line) in lines.iter().enumerate() {
            let entry: HistoryEntry = serde_json::from_str(line).expect("parse entry");
            assert_eq!(entry.text, format!("Message {}", i + 1));
        }
    }

    #[tokio::test]
    async fn test_history_metadata_counts_entries() {
        let temp_dir = TempDir::new().expect("create temp dir");
        let conversation_id = ConversationId::new();
        ...
        let (log_id, count) = history_metadata(temp_dir.path()).await;
        assert!(log_id > 0, "log_id should be non-zero");
        assert_eq!(count, 5);
    }
    ...
    async fn test_lookup_reads_history_entries() {
        ...
        std::fs::create_dir_all(temp_dir.path()).expect("create dir");
    ...
    #[tokio::test]
    async fn test_lookup_returns_none_for_invalid_offset() {
        let temp_dir = TempDir::new().expect("create temp dir");
        let conversation_id = ConversationId::new();
