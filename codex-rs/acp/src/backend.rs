...
impl AcpBackend {
    ...
    /// Get the available models from the agent (if supported).
    ///
    /// Returns `None` if the agent doesn't support model switching.
    pub fn available_models(&self) -> Option<Vec<acp::ModelInfo>> {
        self.connection
            .model_state()
            .map(|s| s.available_models.clone())
    }

    /// Get the current model ID from the agent (if supported).
    ///
    /// Returns `None` if the agent doesn't support model switching.
    pub fn current_model_id(&self) -> Option<acp::ModelId> {
        self.connection.model_state().map(|s| s.current_model_id)
    }

    /// Check if the agent supports model switching.
    pub fn supports_model_switching(&self) -> bool {
        self.connection.model_state().is_some()
    }

    /// Set the model for the current session.
    ///
    /// Returns an error if the model ID is not in the available models list.
    pub async fn set_model(&self, model_id: acp::ModelId) -> Result<()> {
        ...
        if let Some(models) = self.available_models() {
            ...
            anyhow::bail!("Agent does not support model switching");
        }
...
mod tests {
    ...
    fn test_translate_tool_call_to_exec_command_begin() {
        let update = acp::SessionUpdate::ToolCall(
            acp::ToolCall::new("call-123", "shell")
    ...
    fn test_non_text_content_produces_no_events() {
        let update = acp::SessionUpdate::AgentMessageChunk(acp::ContentChunk::new(
            acp::ContentBlock::Image(acp::ImageContent::new("", "image/png")),
        ));
