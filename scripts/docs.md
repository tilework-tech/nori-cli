...
- The script creates synthetic commits via the GitHub API that modify `@/codex-rs/Cargo.toml` with the release version, then tags those commits; this keeps the `dev` branch's `Cargo.toml` at `0.0.0` permanently
