...
- `MOCK_AGENT_MCP_HTTP` -- when set, the agent advertises HTTP MCP capability so `@/nori-rs/acp/src/backend/thread_goal_mcp.rs` and `@/nori-rs/acp/src/connection/local_mcp.rs` can be tested through the normal `session/new` MCP server advertisement path
