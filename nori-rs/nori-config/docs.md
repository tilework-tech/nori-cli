...
- Sandbox mode is resolved into the concrete runtime sandbox policy, and project trust supplies the default approval policy when no explicit policy is present.
- `NoriConfigEdits` applies focused or dotted-path mutations with atomic file replacement while preserving existing TOML comments and formatting. MCP persistence replaces the complete table using the protocol-owned config type.
