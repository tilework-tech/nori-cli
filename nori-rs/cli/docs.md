...
- `nori cloud --onboard` is the CLI half of customer onboarding (Part 2, after the hosted checkout instructions at norisessions.com/onboard-checkout.md have provisioned the org): it additionally sets the clap-skipped `TuiCli.cloud_onboard` flag. The TUI skips the picker but still probes `session/list`; when the broker-tagged onboarding session is present, it dispatches the ordinary ACP resume action so the harness selects `session/load` and replays recorded history. If the purpose tag is absent or the probe is unavailable or fails, it falls back to spawning `cloud-acp --onboard`, preserving the broker's serialized acquire-or-resume behavior for a fresh onboarding session and compatibility with older handroll/broker versions. `/new` repeats this probe-first choice. The onboarding argv remains baked into the process-wide registry entry, so every fallback acquisition is onboarding-only; `/close` remains picker-first
