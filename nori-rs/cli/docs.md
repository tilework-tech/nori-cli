...
- Cloud session lifecycle rides ordinary ACP capabilities, not cloud-specific transport plumbing: the handroll agent advertises `sessionCapabilities.{list,resume,close}` with `loadSession: false`, so the session pickers list broker sessions and reattach live via `session/resume` (no history replay), and `/close` releases the session via `session/close` and returns to the picker. The one-active-session contract (close before claiming another) is enforced agent-side. See `@/nori-rs/harness/docs.md` (resume strategy selection, pre-session probe) and `@/nori-rs/tui/docs.md` (`/resume`, `/close`, picker-first entry, exit-is-detach)
