...
const CODEX_APPLY_PATCH_ARG1: &str = "--codex-run-as-apply-patch";
...
///
/// 1.  Load `.env` values from the Nori home before creating any threads.
...
/// Load env vars from the Nori home `.env` file.
///
/// Security: Do not allow `.env` files to create or modify any variables
/// with names starting with `CODEX_`.
fn load_dotenv() {
    if let Ok(nori_home) = nori_config::find_nori_home()
        && let Ok(iter) = dotenvy::from_path_iter(nori_home.join(".env"))
    {
        set_filtered(iter);
    }
}
