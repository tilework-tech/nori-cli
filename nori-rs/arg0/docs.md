...
Used as the entry point wrapper for `@/nori-rs/tui/` (the `nori` binary). Before the TUI starts, this crate checks if the binary was invoked as `codex-linux-sandbox` or `apply_patch` and dispatches accordingly.
