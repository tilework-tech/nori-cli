...
The TUI drives user interaction through a Ratatui-based interface. When using ACP mode (the primary mode for Nori), user prompts flow through `nori-acp` which spawns and communicates with agent subprocesses over stdin/stdout using JSON-RPC 2.0. Configuration is loaded from `~/.nori/cli/config.toml` when the `nori-config` feature is enabled.

Architecture:
- nori-tui (TUI) -> Terminal User Interface
  - nori-acp -> ACP Agent Connection -> External ACP Agents (claude, etc)
