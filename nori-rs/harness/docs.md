...
**Agent-supplied detail:** `enhance_agent_error()` treats the structured `data.detail` as more precise than static registry hints. For `Authentication` errors the detail (e.g. the deployment's exact login command from `nori-handroll`) replaces the registry's static `auth_hint`; for every other category the detail is appended verbatim in parentheses (e.g. "the session is no longer claimed").
