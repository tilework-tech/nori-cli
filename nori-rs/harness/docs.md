...
The "must appear among the advertised values" check makes the breadth of the agent's advertised list load-bearing for `[default_models]`: a persisted id the agent never advertises is silently dropped, not sent. For Claude this is why the spawn-time model-list widening in `@/nori-rs/acp-host/src/claude_models/docs.md` matters -- it enlarges the advertised set the harness validates against, so ids outside the adapter's short curated list can resolve. The harness itself is unaware of that mechanism; it only ever sees whatever `session/new` returned.
...
- Config options use `SessionConfigOptionCategory` to tag their purpose. The `Mode` category drives the footer mode indicator and `Shift-Tab` cycling. The `Model` category is the only mechanism for model selection -- the TUI's `/model` command opens the value picker on a Model-category config option when the agent advertises one, otherwise it shows a "not supported" fallback (see `@/nori-rs/tui/docs.md`). Real ACP agents like Claude Code provide model selection through this config_options path.
...
**ACP Error Categorization** (`@/nori-rs/acp-host/src/error_category.rs`, re-exported as `nori_acp_host::{AcpErrorCategory, categorize_acp_error}`; user-facing message composition via `enhanced_error_message()` stays in `backend/mod.rs`):
...
- `nori-harness` has no `unstable` feature of its own; the removed one previously gated only the deleted `session/set_model`/`SessionModelState` API. It does enable the *schema's* `unstable` umbrella unconditionally (`agent-client-protocol-schema = { features = ["unstable"] }` in `Cargo.toml`), but not for the reason one might assume: `SessionConfigOptionCategory` carries no cfg gate and its `Model` variant is always available (only the sibling `ModelConfig` variant is gated, behind `unstable_model_config_category`). The feature is genuinely required for `SessionConfigOptionValue` -- the payload type `set_config_option()` builds -- which is gated behind `unstable_boolean_config`. Dropping the feature would break `set_config_option`, not the Model category
