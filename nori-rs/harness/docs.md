...
- **Graceful stdin-EOF shutdown:** `shutdown()` closes the child's stdin and waits a grace period before killing the process group; `nori-handroll cloud-acp` relies on this to release its broker session.
