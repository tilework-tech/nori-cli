...
- The stdin-EOF-then-grace shutdown contract exists because agents may need network cleanup on exit; `nori-handroll cloud-acp` detaches from its broker session on stdin EOF (a non-terminal signal -- the cloud session keeps running, and only ACP `session/close` releases it), and the previous immediate-SIGKILL shutdown broke every clean detach. `shutdown_with_grace()` lets callers size the grace: the harness's pre-session probe (`@/nori-rs/harness/src/backend/probe.rs`) uses a short one because its child never created a session
...
- `list_sessions()` sends the ACP `session/list` request and drains cursor pagination internally (following `next_cursor` until exhausted, bounded by a page cap so a misbehaving agent that keeps returning a cursor cannot loop unbounded), concatenating every page in agent order. Each ACP `SessionInfo` is mapped into `AcpSessionSummary` (defined in `mod.rs`, re-exported from `nori_harness`), an owned boundary type carrying `session_id`, `cwd`, optional `title`, optional `updated_at`, and optional `meta` (the row's ACP `_meta` extension object, passed through as a `serde_json::Value`). The boundary type decouples consumers from the raw ACP schema, letting `@/nori-rs/tui` -- which has no ACP-schema dependency -- render the agent-sourced `/resume` picker. It mirrors `load_session()`'s structure; the agent only honors `session/list` when it advertises the capability (projected as `agent.session_list`, see `@/nori-rs/harness/docs.md`). `AcpSessionSummary` derives `PartialEq` but not `Eq` because `meta` is an arbitrary JSON value that may contain floats. The resume picker in `@/nori-rs/tui/src/nori/resume_session_picker.rs` reads `meta.nori.origin == "cloud"` to identify cloud sessions (whose `cwd` is not a real filesystem path) instead of matching on a magic cwd value; see `@/nori-rs/tui/docs.md` for the transition shim that still recognizes the legacy sentinel
