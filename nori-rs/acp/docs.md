...
When the TUI sends `Op::RunUserShellCommand` (from prompt-initial `!cmd`), the ACP backend runs the command locally in the session working directory using the user's shell. This is intentionally local Nori behavior rather than an ACP agent request: the backend emits `TaskStarted`, `ExecCommandBegin`, any stdout/stderr `ExecCommandOutputDelta` events, `ExecCommandEnd`, and finally `TaskComplete` so the shared TUI exec cell path renders the result and returns the composer to prompt state.
