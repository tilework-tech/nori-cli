...
`BrowserSession::launch_and_store()` finds a Chrome or Chromium binary via the `which` crate (searching `google-chrome-stable`, `google-chrome`, `chromium-browser`, `chromium`), then spawns it with `--remote-debugging-port=0` (OS-assigned port) against a throwaway `tempfile::TempDir` profile (`--user-data-dir`). The isolated profile is deliberate: reusing the default profile would hand off to the user's already-running Chrome and never expose CDP on our debugging port. It parses the CDP WebSocket URL from Chrome's stderr output (the `DevTools listening on ws://...` line) with a 15-second timeout via `tokio::time::timeout`. Helper functions `parse_cdp_ws_url()` and `extract_cdp_port()` handle the stderr parsing and port extraction; the prompt/screen CDP-port parser shared with the test crates lives separately in `backend/browser_cdp.rs`.
