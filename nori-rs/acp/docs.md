...
**Retryable classification and the `ErrorEvent.retryable` flag:** `AcpErrorCategory::is_retryable()` returns `true` only for `ApiServerError` and `QuotaExceeded` -- transient server errors and rate/quota limits that clear on their own -- and `false` for the persistent categories a retry cannot fix. This drives the `retryable: bool` field on `codex_protocol`'s `ErrorEvent` (see `@/nori-rs/protocol/src/protocol/mod.rs`), which is `#[serde(default)]` so any producer that does not set it defaults to `false`, preserving the historical "any error is fatal" behavior. `send_prompt_error` sets `ErrorEvent.retryable = category.is_retryable()` only for user prompts (`QueuedPromptKind::User`); `Compact` and `GoalContinuation` failures are always fatal (`retryable = false`). The other `ErrorEvent` producers -- agent process exit in `backend/spawn_and_relay.rs` and the generic `send_error` op path in `backend/submit_and_ops.rs` -- also emit `retryable = false`. The TUI consumes this flag so an unattended loop-count run survives a momentary API blip instead of silently stopping; the loop-cancel-vs-retry decision lives in `@/nori-rs/tui/docs.md` (Loop Mode section). `send_prompt_error` logs the category, the resolved `retryable`, and the raw error for observability.
