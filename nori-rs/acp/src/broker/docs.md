...
- Cloud mode in both `AcpBackend::spawn()` and `AcpBackend::resume_session()` skips agent config lookup (`get_agent_config`) since the remote agent is already running on the cloud VM. Error messages for cloud connection failures use simple messages instead of the enhanced error categorization used for local subprocess failures. Both spawn paths pass `config.cloud_connection.is_some()` as `is_cloud` to `TranscriptRecorder::new()`, which persists the flag in the transcript `session_meta` for downstream consumers (resume pickers, cloud session warnings)
