...
Path: @/nori-rs/process-hardening
...
Called via `#[ctor::ctor]` attribute to run before `main()` in `@/nori-rs/tui/`. Ensures the CLI process starts in a hardened state to protect API keys and sensitive data.
