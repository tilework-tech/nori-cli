...
- The local callback server accepts only `/callback` requests containing both `code` and `state` query parameters; `@/nori-rs/rmcp-client/src/perform_oauth_login.rs` percent-decodes those values before handing them to `rmcp::OAuthState` or the direct `oauth2` token exchange path.
