...
The `desc_col` is computed once per render pass from the widest name in the complete filtered result set plus 2 columns of padding. It therefore remains stable while the viewport scrolls. The stacked fallback prevents descriptions from being squeezed into 1-2 characters of horizontal space on narrow terminals. Because both `render_rows()` and `measure_rows_height()` use the same column and call the same `wrap_row()` function, layout and height calculation remain consistent.
...
`set_text()` deliberately preserves the kill/yank buffer while clearing visible text, elements, and undo/redo state. Submission and slash-command clears can therefore be followed by Ctrl-Y or Vim paste without losing the most recent kill. History-specific replacement is separate from generic composer prefilling: recalled local, remote, and search results place the cursor at the end, while programmatic prefills retain the generic cursor-at-start behavior.
...
**Paste Ownership and Placeholders:**

Paste normalization and terminal-control sanitization belong to `ChatComposer`, the boundary shared by bracketed paste, shell-mode paste, and paste-burst flushes. CRLF is normalized before bare CR so Windows newlines remain single newlines; CSI/control sequences are removed while Unicode, tabs, and newlines are preserved. Large payloads stay outside the editable buffer behind atomic placeholders. Pending pastes with equal character counts receive unique suffixes (`#2`, `#3`, and so on), and one expansion path reconstructs both normal and shell submissions without rescanning inserted payload text.
...
`/quit`, `/exit`, and idle Ctrl+C all route through the idempotent `ChatWidget::begin_exit()` transition instead of submitting `Op::Shutdown` directly. Before graceful shutdown begins, `ChatWidget -> BottomPane -> ChatComposer` enters one authoritative disabled-composer state. It closes active bottom-pane views and composer popups, clears buffered paste-burst input and transient footer hints, preserves but hides the draft and attachments, hides the cursor/footer, and renders only the dim `› Exiting…` row. The same state refuses keys, explicit paste, paste images, programmatic insertion/prefill, delayed paste-burst flushes, and the task-status Esc interrupt path, so no interactive mutation or backend operation can start during teardown.

Local exit does not add a history cell. For agents with the cloud lifecycle (`session_resume && !load_session`), history receives only the durable distinction the composer cannot express: "This session keeps running in the cloud." plus the `nori cloud` reattach hint. After feedback is visible, `begin_exit()` submits `Op::Shutdown` and starts the hard-exit watchdog, which force-sends `AppEvent::ExitRequest` after `EXIT_HARD_DEADLINE` (~1s). The watchdog exists because backend child-exit grace can be as long as 25 seconds; post-sessions-#1276 the cloud agent treats connection EOF as a non-terminal detach (`session/close` is the only terminal verb), so there is nothing worth holding the TUI hostage for. A cooperative child still finishes its EOF path, while a stuck child is left to the connection layer's grace logic after the TUI exits. The widget-level `exiting` guard continues to drop direct prompt submissions and refuse slash commands except repeated `/quit`/`/exit`.
