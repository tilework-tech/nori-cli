...
1. `SlashCommand::Browser` in `key_handling.rs` (when no session is already active) loads `NoriConfig` for the saved `browser_profile` default and opens the **browser-profile picker** (`open_browser_profile_picker()` → `config_picker::browser_profile_picker_params()`), pre-highlighted on that default. The picker offers three tiers: `Throwaway` (secure default), `Persistent nori profile`, and `Real Chrome profile`.
2. Selecting a tier emits `AppEvent::SetBrowserProfile(mode)`. The handler in `app/event_handling.rs` persists the choice as the new default via `persist_browser_profile_setting()` (writing `[tui] browser_profile`), then calls `chat_widget.launch_browser_session(mode)`, which shows an info message ("Launching browser...") and spawns a `tokio` task calling `BrowserSession::launch_and_store(mode)` (see `@/nori-rs/acp/docs.md` for the per-tier profile resolution)
3. On success, the task posts `AppEvent::BrowserLaunched { ws_url, cdp_port }`. On failure, it posts `AppEvent::BrowserLaunchFailed(error_string)` (for `System`, the error carries a "quit Chrome first" hint when the real profile is already locked)
4. The `BrowserLaunched` handler in `app/event_handling.rs` calls `browser_session::compose_agent_prompt()` to build a structured message containing the CDP HTTP endpoint and WebSocket URL, then submits it as a user message via `submit_user_message_text()`
5. The agent receives the CDP connection details and can use Playwright, Puppeteer, or raw CDP commands via its shell tool to control the browser

`launch_and_store()` keeps Chrome alive by storing the `BrowserSession` in a process-lifetime `static` in the ACP backend, so it outlives the spawning task and a second `/browser` reuses it. On exit, `run_ratatui_app` (`lib.rs`) calls `browser_session::shutdown_active_session()` to kill Chrome and remove its temp profile (for the throwaway tier; persistent/system profiles are left on disk — see the lifecycle and shutdown notes, including the remaining hard-kill limitation, in `@/nori-rs/acp/docs.md`). This is distinct from `/browse` which opens a terminal file manager.
