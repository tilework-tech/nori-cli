...
The final resolved config is retained as the TUI's runtime snapshot and shared with each `SessionLaunchSpec` as an `Arc<NoriConfig>`. The harness projects all backend settings from that injected value and never loads ambient configuration during launch, resume, or probing. Pickers, slash commands, and scripts also read the runtime snapshot instead of reloading the user file and losing launch overrides. Successful in-TUI edits update both the file and the runtime snapshot, so a later session sees changes such as default models and ACP wire recording immediately.
...
The `desc_col` is computed once per render pass from the widest name in the complete filtered result set plus 2 columns of padding. Measuring the full result set keeps description columns fixed while the viewport scrolls. The stacked fallback prevents descriptions from being squeezed into 1-2 characters of horizontal space on narrow terminals. Because both `render_rows()` and `measure_rows_height()` use the same full-row measurement and call the same `wrap_row()` function, layout and height calculation stay consistent.
...
| Context line  | `context_window_percent` present, with or without token data | "Context: 77.0K (27%)" or just "Context: 42%" |
...
**Argument shortcut:** `/switch-skillset <name>` (e.g., `/switch-skillset foobar`) bypasses the picker entirely and directly triggers the install or switch. This is intercepted in `submit_user_message()` in `chatwidget/user_input.rs` before the text is sent to the model, following the same `strip_prefix` + early-return pattern used by `/login <agent>`. The handler `handle_switch_skillset_command_with_name()` in `chatwidget/pickers.rs` resolves the install dir with the same shared `session_skillset_install_dir()` helper (see below) as the picker flow but skips the async list step, calling `on_switch_skillset_request()` or `on_install_skillset_request()` directly. An empty name after the prefix (e.g., `/switch-skillset ` with trailing space only) is not intercepted and falls through to normal message submission.

Both switch call sites (`handle_switch_skillset_command()` and `handle_switch_skillset_command_with_name()`) delegate to `session_skillset_install_dir()` in `nori/skillset_picker.rs` to resolve `install_dir`. It returns `Some(cwd)` only when `cwd` is a git worktree (its parent directory is named `.worktrees`, detected via `extract_worktree_name()`); otherwise it returns `None` and the switch targets the user's home install. `skillset_per_session` does NOT force a non-worktree cwd to be used as `install_dir` -- doing so would scatter `.nori-managed` markers and skillset manifests into the repo root. The legitimate worktree case is unaffected: when auto-worktree fires (Automatic or Ask-accepted), `config.cwd` is already the `.worktrees/<name>` path, so it is correctly used as `install_dir`.
...
When `skillset_per_session` is on and `auto_worktree` is `Off`, the picker subtitle changes from "Switching skillset in {dir}" to "Warning: skillset files will be added to {dir}" to warn that skillset files will be written directly to the current working directory (no worktree isolation). The `on_skillset_list_result()` method in `pickers.rs` reads both the `show_no_skillset` and `auto_worktree_off` flags from the resolved runtime config.
...
| Context Window            | `context`                   | Default compact context display, such as "44% / 272k"; falls back to the available percentage or used-token count                                                                             |
| Context Used Percent      | `context_used_percent`      | Integer percentage of the current context window in use, such as "44%"                                                                                                                        |
| Context Remaining Percent | `context_remaining_percent` | Integer percentage of the current context window still available, such as "56%"                                                                                                              |
| Context Used Tokens       | `context_used_tokens`       | Current context-window usage as a compact token count, such as "120k"                                                                                                                         |
| Context Remaining Tokens  | `context_remaining_tokens`  | Maximum minus used context tokens, clamped at zero, such as "152k"                                                                                                                            |
| Context Window Tokens     | `context_window_tokens`     | Maximum context-window size as a compact token count, such as "272k"                                                                                                                          |
...
Segment placement is configurable through `[tui.footer_layout]`. Missing layout fields use defaults: status segments render on `footer_left`, and `mode_indicator` renders on `footer_right`. A field that is present replaces that placement; listed built-in segments are moved out of other default placements so a partial override can move one segment without duplicating it. The same layout-entry syntax works in `footer_left`, `footer_right`, `textarea_top_left`, `textarea_top_right`, `textarea_bottom_left`, and `textarea_bottom_right`.

Each layout entry is either a built-in segment key or an inline custom chunk with one `format` string. Placeholders use the same built-in keys from the table above:
...
Custom formats intentionally stay small and deterministic:

- `{segment_name}` embeds one built-in segment; custom chunks cannot reference other custom chunks.
- Referenced spans keep the built-in segment's colors and emphasis. Literal text uses the surrounding default style.
- A reference is independent of its `[tui.footer_segments]` toggle. Those toggles control standalone built-in layout entries only.
- If any referenced segment lacks its runtime data, the entire custom chunk is hidden.
- Unknown built-ins or placeholders and unmatched braces reject the configuration when it loads.
- `{{` and `}}` emit literal braces. Expressions, conditions, nested placeholders, and format specifiers are not supported.

`FooterLayoutItem` and the config-load parser live in `@/nori-rs/nori-config/src/types/mod.rs`; runtime composition lives in `@/nori-rs/tui/src/bottom_pane/footer.rs`. Parsing once at config load leaves rendering as a simple walk over validated literal and built-in parts.

Context data is sourced in priority order in `@/nori-rs/tui/src/bottom_pane/chat_composer/rendering.rs`. ACP `SessionUpdateInfo { kind: Usage, usage: Some(..) }` supplies both used and maximum tokens when available. Otherwise, `TranscriptLocation.token_breakdown` (provided by `nori_harness::discover_transcript_for_agent_with_message()`) supplies used-token data, and the detected agent kind supplies its maximum context-window size. The same resolved values drive the default `context` segment and all five atomic context segments. The transcript breakdown also includes separate input, output, and cached token counts for the independent `token_usage` segment.
...
When `/browse` is invoked, `SlashCommand::Browse` checks `file_manager` in the resolved runtime config. If `None`, an error message directs the user to `/settings`. If set, it sends `AppEvent::BrowseFiles(fm)`.
...
`/quit`, `/exit`, and idle Ctrl+C all route through the idempotent `ChatWidget::begin_exit()` in `@/nori-rs/tui/src/chatwidget/helpers.rs` instead of submitting `Op::Shutdown` directly. The transition first sets the widget's `exiting` flag and asks `BottomPane` to enter one authoritative disabled-composer state. That state preserves but hides the draft, closes bottom-pane views and composer popups, clears buffered paste-burst input and transient hints, and hides the cursor and footer. Key gating happens one level up: `BottomPane::handle_key_event` in `@/nori-rs/tui/src/bottom_pane/mod.rs` early-returns while the composer is disabled, so every key is inert -- including the task-status Esc shortcut that would otherwise emit `Op::Interrupt` mid-teardown. The composer likewise refuses paste, image, flush, programmatic `set_text_content`, and Ctrl+C-clear mutations, so nothing (not even a second Ctrl+C) can touch the preserved hidden draft. Rendering takes a dedicated early-return path that draws only the dim `› Exiting…` prompt and placeholder -- no popups, footer, or textarea -- instead of threading disabled conditionals through the enabled render. Local exit adds no history cell. Agents with the cloud lifecycle -- `session_resume && !load_session` -- retain a separate history note, "This session keeps running in the cloud.", with a reattach hint pointing at the `nori cloud` picker.

After the UI transition, `begin_exit()` submits `Op::Shutdown` for graceful backend teardown and spawns a hard-exit watchdog that force-sends `AppEvent::ExitRequest` after `EXIT_HARD_DEADLINE` (~1s). The watchdog exists because the backend's child-exit grace can be as long as 25 seconds; post-sessions-#1276 the cloud agent treats connection EOF as a non-terminal detach (`session/close` is the only terminal verb), so there is nothing worth holding the TUI hostage for -- a cooperative child still finishes its EOF path, and a stuck one gets SIGKILLed by the connection layer's grace logic after the TUI is gone. The ACP shutdown timing and teardown semantics are unchanged. While `exiting` is set, `submit_user_message()` in `@/nori-rs/tui/src/chatwidget/user_input.rs` drops prompts and `dispatch_command()` in `@/nori-rs/tui/src/chatwidget/key_handling.rs` refuses every slash command except repeated `/quit`/`/exit`; the disabled bottom pane and composer provide the corresponding lower-level input and mutation gates.
