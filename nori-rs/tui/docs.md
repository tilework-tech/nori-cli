...
**Custom Model Input** (`nori/custom_model_input.rs`, `nori/session_config_picker.rs`): When the value picker is for a Model-category config option, `acp_session_config_value_picker_params()` appends a "Use custom model..." entry at the end of the option list. Selecting it emits `AppEvent::OpenCustomModelInput { config_id, option_name }`, which `app/event_handling.rs` routes to `ChatWidget::open_custom_model_input()` in `chatwidget/pickers.rs`. That method creates a `CustomModelInputView` -- a `BottomPaneView` that presents a bordered text input for free-form model ID entry. On submit, the view emits `AppEvent::SetAcpSessionConfigOption` with the typed value as both `value` and `value_name`, which follows the same `session/set_config_option` RPC path and model persistence logic as picking a value from the agent's advertised list. Empty or whitespace-only input is rejected without emitting. Esc dismisses the input without side effects. Non-model config option pickers (mode, thought level) do not include the custom entry.
...
The `/resume` command allows reconnecting to a previous ACP session. It uses the ACP agent's `session/load` RPC when available, and otherwise falls back to a fresh ACP session plus normalized replay derived from the saved transcript (see `@/nori-rs/harness/docs.md`).
