...
The loop survives transient failures and cancels only on fatal ones. On a prompt failure the ACP backend emits both an `EventMsg::Error` and a `PromptCompleted { Cancelled }`; if every error unconditionally cancelled the loop, the error would disarm it before the completion could re-fire the next iteration, silently stopping the run on a momentary API blip (e.g. Anthropic `529`/overloaded or a rate limit). To prevent this, `ErrorEvent` carries a `retryable` flag (set by the ACP backend's `send_prompt_error`, see `@/nori-rs/acp/docs.md`) and `on_error(message, retryable)` in `@/nori-rs/tui/src/chatwidget/event_handlers.rs` calls `cancel_loop()` only when `!retryable`. A retryable error leaves the loop armed, and the subsequent `PromptCompleted`-driven `on_task_complete` re-fires the next iteration; a fatal error disarms both `loop_remaining` and `loop_total`. `cancel_loop()` (in `@/nori-rs/tui/src/chatwidget/pickers.rs`) is a no-op when no loop is active and logs (`tracing::info`) only when it actually cancels. The `/settings` sub-picker is a custom `BottomPaneView` implemented by `LoopCountPickerView` in `@/nori-rs/tui/src/nori/loop_count_picker.rs`. It offers preset options (Disabled, 2, 3, 5, 10) plus a "Custom..." option that enters an input mode where the user can type an arbitrary number (2-1000). Values <= 1 are treated as disabled, values > 1000 are capped. This follows the same `BottomPaneView` pattern used by `HotkeyPickerView`. The setting persists to `[tui]` in `config.toml` via `persist_loop_count_setting()`.
