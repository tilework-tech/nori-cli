...
| Dev branch push | Push to `dev` (e.g., merged PR) with path filters | `publish_next=true` -- publishes a `@next` dev snapshot |
| Manual dispatch | `workflow_dispatch` with inputs | Either `publish_next=true` or explicit version + optional `dry_run` |

**Path filters for dev branch pushes** restrict triggering to changes in `codex-rs/**`, `nori-cli/**`, `scripts/**`, and the workflow file itself, so docs-only changes do not trigger a release.

**Concurrency control** uses group `nori-release-${{ github.ref }}` with `cancel-in-progress` enabled only for dev branch pushes. This means if two PRs merge in quick succession, the second run cancels the first since only the latest snapshot matters.
...
- The `validate` job distinguishes tag pushes from dev branch pushes using `startsWith(github.ref, 'refs/tags/')` rather than just checking `github.event_name == 'push'`, since both tag pushes and dev branch pushes have `event_name == 'push'`
