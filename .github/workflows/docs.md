...
- Build runners use Blacksmith (e.g., `blacksmith-4vcpu-ubuntu-2404`) for most jobs, with a GitHub-hosted `ubuntu-24.04` runner for `publish-npm`. The publish job uses npm OIDC Trusted Publishing, which auto-generates a sigstore provenance attestation; the npm registry only accepts provenance from GitHub-hosted runners and rejects self-hosted/Blacksmith runners (`runner_environment: self-hosted`). Other jobs can stay on Blacksmith since only the publishing job's runner is verified for provenance.
